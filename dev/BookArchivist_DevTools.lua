...
local function createGridOverlay(frame, r, g, b, label)
	...
	return { region = overlayRegion, fill = fill, edges = edges, labelFrame = labelFrame, labelText = labelText, label = label, frame = frame }
end
...
local function hookMainSlashCommands()
	...
	SlashCmdList["BOOKARCHIVIST"] = function(msg)
		...
		if verb == "modules" or verb == "modstatus" then
			...
			chatMessage(string.format("  FramePool: %s", tostring(BookArchivist and BookArchivist.UI and BookArchivist.UI.FramePool ~= nil)))
			chatMessage(string.format("  TestDataGenerator: %s", tostring(BookArchivist and BookArchivist.TestDataGenerator ~= nil)))
		...
		if verb == "iter" or verb == "iterator" then
			...
			if subCmd == "test" then
				...
				Iterator:Start(
					"test_iteration",
					BookArchivist:GetDB().booksById or {},
