...
function Generator:GenerateLocation(index)
  local zones = {
    "Stormwind City", "Orgrimmar", "Ironforge", "Thunder Bluff",
    "Darnassus", "Undercity", "Silvermoon City", "The Exodar",
    "Dalaran", "Boralus", "Dazar'alor",
  }
  
  local subzones = {
    "Library", "Great Hall", "Market District", "Cathedral Square",
    "Valley of Wisdom", "War Quarter", "Trade District",
  }
...
function Generator:PrintStats()
  ...
  print(string.format("  Books with authors: %d (%.1f%%)", 
    stats.withAuthors, (stats.withAuthors / stats.totalBooks) * 100))
  print(string.format("  Books with locations: %d (%.1f%%)", 
    stats.withLocations, (stats.withLocations / stats.totalBooks) * 100))
  print(string.format("  Favorite books: %d (%.1f%%)", 
    stats.favorites, (stats.favorites / stats.totalBooks) * 100))
...
function Generator:GeneratePreset(preset)
  ...
  if not config then
    ...
    print("Available presets: small (100), medium (500), large (1000), xlarge (2500), stress (5000), minimal (50), rich (200)")
    return
  end
