...
Locales.enUS = {
  ...
  ["OPTIONS_IMPORT_PERF_TIP"] = "Import options on this client:\n\n- Quick self-test: Click Export, then click Import here. The addon reuses the in-memory export; no copy/paste needed.\n- Same client after Copy: Click Export, then Copy, then (optionally) Capture Paste and press Ctrl+V once. This lets you test exactly what was copied without retyping it.\n- From another client/account: On the source, Export then Copy; on the target, make sure that export text is in your OS clipboard, open this panel, click Capture Paste, then press Ctrl+V once, wait for 'Payload received', then click Import.\n\nImportant: Capture Paste cannot read your clipboard by itself. It only tells the addon to listen for the *next* Ctrl+V paste you perform in-game; without that manual paste, nothing is imported.\n\nPerformance warning: Importing a large export string can temporarily freeze the game client while WoW processes the paste and the books are decoded. For example, importing around 10–15 books may pause the game for 10–15 seconds; importing 50 or more books may take close to a minute, and very large payloads may cause the paste to fail or even crash the client due to engine limitations. Whenever possible, prefer smaller exports or per-character imports instead of extremely large, all-in-one imports.",
