				...
				if #tokens > 0 then
					...
					for i = 1, #tokens do
						...
						if token ~= "" then
							...
							if
								(entry.searchText or ""):lower():find(token, 1, true)
								and not titleHaystack:find(token, 1, true)
							then
								anyText = true
							end
						end
...
local function createTestBooks(count)
	...
	for i = 1, count do
		...
		if BookArchivist.Search and BookArchivist.Search.BuildSearchText then
			books[bookKey].searchText = BookArchivist.Search.BuildSearchText(books[bookKey].title, books[bookKey].pages)
		end
	...
	describe("Search and Filtering (Real Logic)", function()
		it("should build searchText from title and pages", function()
			...
			for _, bookKey in ipairs(order) do
				...
				assert.is_true(
					book.searchText:find(lowerTitle, 1, true) ~= nil,
					"searchText should contain lowercase title"
				)
				assert.is_true(
					book.searchText:find("page one", 1, true) ~= nil,
					"searchText should contain page content (lowercase)"
				)
			end
		end)
		...
		it("should filter by search query (content match)", function()
			...
			books["book3"].searchText = BookArchivist.Search.BuildSearchText(books["book3"].title, books["book3"].pages)
		...
		it("should filter by multiple search tokens (AND logic)", function()
			...
			books["book7"].searchText = BookArchivist.Search.BuildSearchText(books["book7"].title, books["book7"].pages)
		...
		it("should track match kinds (title vs content)", function()
			...
			books["book5"].searchText = BookArchivist.Search.BuildSearchText(books["book5"].title, books["book5"].pages)
		...
		it("should handle 1000 books with complex search", function()
			...
			for i = 1, 1000 do
				...
				if i % 10 == 0 then
					...
					books[bookKey].searchText =
						BookArchivist.Search.BuildSearchText(books[bookKey].title, books[bookKey].pages)
				end
			end
