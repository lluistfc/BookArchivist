...
function Profiler:Stop(label)
  ...
  if not label or not startTimes[label] then return 0 end
...
function Profiler:Report(sortBy)
  ...
  local lines = {
    ...
    "",
    string.format("%-30s %8s %8s %8s %8s %8s %10s", 
      "Operation", "Count", "Avg(ms)", "Max(ms)", "Min(ms)", "Last(ms)", "Total(ms)"),
    string.rep("-", 100),
  }
...
function Profiler:PrintSummary()
  ...
  for i, op in ipairs(slowest) do
    table.insert(lines, string.format("    %d. %s: %.2fms (n=%d)", 
      i, op.label, op.avg, op.count))
  end
