...
local function BookArchivistChatFilter(self, event, msg, sender, ...)
	...
	newMsg = msg:gsub("%[BookArchivist: (.+)%]", function(title)
		...
		return string.format(
			"|Hbookarc:%s:%s|h|cFF%s[BookArchivist: %s]|h|r",
			encodedSender,
			encodedTitle,
			LINK_COLOR,
			title
...
function ChatLinks:ShowImportWithData(bookData)
	...
	self.promptTitle:SetText(
		string.format(L["IMPORT_PROMPT_TITLE_WITH_DATA"] or "Import: %s", bookData.title or "Book")
	)
	...
	local confirmMsg = (L["IMPORT_CONFIRM_MESSAGE"] or "Book received! Click Import to add '%s' to your library."):format(
		bookData.title or "this book"
	)
...
function ChatLinks:CreateImportPrompt()
	...
	local instructionText = (L["IMPORT_PROMPT_TEXT"] or "Paste the book export string below:")
		.. "\n\n"
		.. (
			L["IMPORT_PROMPT_HINT"]
			or "The sender must share the full export string with you separately (outside WoW chat)."
		)
...
function ChatLinks:DoImport()
	...
	if self.promptEditBox and self.promptEditBox.GetText then
		...
		BookArchivist:DebugPrint(
			"|cFFFF0000BookArchivist:|r "
				.. (L["IMPORT_FAILED"]:format("No text provided") or "Import failed: No text provided")
		)
	...
	if not ImportWorker then
		BookArchivist:DebugPrint(
			"|cFFFF0000BookArchivist:|r "
				.. (
					L["IMPORT_FAILED"]:format("Import system unavailable") or "Import failed: Import system unavailable"
				)
		)
