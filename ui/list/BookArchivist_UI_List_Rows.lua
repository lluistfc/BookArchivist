...
function ListUI:UpdateList()
	...
	if mode == modes.BOOKS then
		...
		if total == 0 and dbCount > 0 and not isFilteredCategory then
			self:DebugPrint(
				string.format(
					"[BookArchivist] updateList: filtered empty but DB has %d books, forcing rebuild",
					dbCount
				)
			)
		...
		self:DebugPrint(
			string.format("[BookArchivist] UpdateList: Added %d books to data provider", dataProvider:GetSize())
		)
	...
	self:DebugPrint(
		string.format("[BookArchivist] UpdateList: Added %d locations to data provider", dataProvider:GetSize())
	)
