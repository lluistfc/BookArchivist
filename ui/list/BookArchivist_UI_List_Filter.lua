	...
	if Iterator and #baseKeys > 0 then
		...
		if self.__state.isAsyncFiltering then
			...
			if elapsed > 30 then
				self:DebugPrint(
					string.format(
						"[BookArchivist] rebuildFiltered: async filter stuck for %.1fs, forcing reset",
						elapsed
					)
				)
		...
		local success, errorMsg = Iterator:Start(
			...
			end,
			{
				...
				onProgress = function(progress, current, total)
					...
					if noResults then
						noResults:SetText(
							string.format("|cFFFFFF00Filtering: %d/%d (%.0f%%)|r", current, total, progress * 100)
						)
					end
				...
				onComplete = function(context)
					...
					local success, err = pcall(function()
						...
						if noResults and #filteredKeys == 0 then
							noResults:SetText(
								"|cFF999999" .. (self.L and self.L["LIST_EMPTY_SEARCH"] or "No results") .. "|r"
							)
